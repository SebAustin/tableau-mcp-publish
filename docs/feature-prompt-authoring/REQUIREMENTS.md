...
| E2E-1 | `build_from_plan` requires `datasourceSpec.filePath`: it builds the datasource (1× `buildDatasourceFromFile` + 1× `rest.publishDatasource`), calls the sidecar `/workbook/dashboard` endpoint exactly once, and calls `rest.publishWorkbook` exactly once. A plan with no `datasourceSpec` (LUID-only) or a `sql`-only spec is rejected with an actionable error (it cannot embed an extract, so Tableau Cloud would return 400011) before any build or publish. | TS vitest with mocked rest + sidecar | headless |
